  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to="/category/add" className="bg-indigo-600 text-white px-5 py-2 rounded-lg shadow hover:bg-indigo-700 transition">Add Category</Link>
      </div>
      {loading ? (
            ...
            {categories.map(category => (
              <div key={category._id} className="bg-white dark:bg-gray-900 shadow-lg rounded-2xl p-4 flex flex-col hover:shadow-2xl transition-shadow">
                ...
                <div className="mt-auto flex gap-2">
                  <Link to={`/category/${category._id}`} className="px-3 py-1 bg-indigo-100 text-indigo-700 rounded hover:bg-indigo-200 transition">View</Link>
                  <Link to={`/category/${category._id}/edit`} className="px-3 py-1 bg-yellow-100 text-yellow-700 rounded hover:bg-yellow-200 transition">Edit</Link>
